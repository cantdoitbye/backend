...
class Query(graphene.ObjectType):
    ...
    def resolve_my_feed(self, info,circle_type=None):
        ...
        filtered_posts = filter(
            lambda post: (
                ...
                post.created_by.single().user_id != str(user_id)  # Exclude posts by logged-in user
            ),
            all_posts
        )
    ...
    def resolve_recommended_post(self,info):
        ...
        details=["Top Vibes - Meme","Top Vibes - Podcasts","Top Vibes - Videos","Top Vibes - Music","Top Vibes - Articles","Post From Connection","Popular Post","Recent Post"]
...
class QueryV2(graphene.ObjectType):
    ...
    def resolve_my_feed_test(self, info, circle_type=None):
        ...
        for post in results:
            ...
            # Append for all posts if no circle_type is selected or its value is null
            if circle_type is None or original_circle_type == circle_type.value:
                result_feed.append(FeedTestTypeV2.from_neomodel(post_node,likes,connection,circle,user_node,profile_node,user_relations,login_user_uid))
